...
class TestDataGenerator {
  private domains = ['example.com', 'testsite.org', 'mydomain.net'];
  private commonPaths = [
    '/',
    '/about',
    '/contact',
  ...
  private generateNormalTraffic(): LogLine[] {
    const logs: LogLine[] = [];
    const now = new Date();
  ...
  private generateAttackScenario(): LogLine[] {
    const logs: LogLine[] = [];
    const now = new Date();
  ...
  private generatePerformanceIssues(): LogLine[] {
    const logs: LogLine[] = [];
    const now = new Date();
  ...
  private generateErrorSpike(): LogLine[] {
    const logs: LogLine[] = [];
    const now = new Date();
  ...
  private generateBotCrawling(): LogLine[] {
    const logs: LogLine[] = [];
    const now = new Date();
    const botUserAgents = [
      'Googlebot/2.1 (+http://www.google.com/bot.html)',
  ...
  private getTrafficForHour(hour: number): number {
  ...
  private generateReferer(): string {
    const referers = [
  ...
  private async writeLogsToFiles(scenarioName: string, logs: LogLine[]): Promise<void> {
    const testDataDir = path.join(__dirname, '../test/testdata');
  ...
  private formatAsNginxLog(log: LogLine): string {
  ...
  private formatAsNginxErrorLog(log: LogLine): string {
    ...
    const message = errorMessages[log.status as keyof typeof errorMessages] || 'unknown error';
    return `${timestamp} [error] 12345#0: ${message}: "${log.path}", client: ${log.ip}, server: example.com, request: "${log.method} ${log.path} HTTP/1.1"`;
  }
}

export async function generateTestLogs(): Promise<void> {
