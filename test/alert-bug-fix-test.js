 ...
 *
 * BUG DESCRIPTION:
 * The checkImmediateAlerts() method was calling getDomain(entry.domain_id.toString())
 * which passes a domain ID (e.g., "1") instead of a domain name (e.g., "example.com").
 * This caused getDomain() to always return null, breaking the immediate alert system.
 *
 * FIX:
 * - Added getDomainById(id: number) method to Storage class
 * - Updated alerts/index.ts to use getDomainById(entry.domain_id)
 *
 * This test verifies:
 * 1. getDomainById() correctly retrieves domain by ID
 * 2. Immediate alerts are triggered when 10+ 500 errors occur in 5 minutes
 * 3. The bug is fixed and the alert system works correctly
 */

const fs = require('fs');
const path = require('path');

// Since TypeScript compilation has errors, we'll test by compiling only the needed files
...
async function runTest() {
  ...
  try {
    // Compile only the files we need for testing
    console.log('📦 Compiling TypeScript files...');
    execSync('npx tsc src/storage/index.ts --outDir dist --module commonjs --target ES2020 --esModuleInterop --skipLibCheck --resolveJsonModule --declaration', {
      stdio: 'ignore'
    });
    execSync('npx tsc src/types/index.ts --outDir dist --module commonjs --target ES2020 --esModuleInterop --skipLibCheck --resolveJsonModule --declaration', {
      stdio: 'ignore'
    });
    ...
    console.log(`  getDomainById(${testEntry.domain_id}) = Domain { id: ${domainAfterFix?.id}, name: "${domainAfterFix?.name}" }`);
