...
const security_routes_1 = require("./security-routes");
function createServer(config, storage, discovery, parser) {
    ...
    const analyzer = new analyzer_1.LogAnalyzer(storage);
    const metricsCollector = new metrics_1.MetricsCollector(storage, analyzer);
    const authManager = new auth_1.AuthManager(storage);
    // Initialize security middleware
    const security = (0, security_1.createSecurityMiddleware)({
        storage,
    ...
    app.use((0, cors_1.default)({
        origin: config.security?.cors?.origin || true,
        credentials: true,
    ...
    // Security routes
    const securityRoutes = (0, security_routes_1.createSecurityRoutes)(storage, authManager);
