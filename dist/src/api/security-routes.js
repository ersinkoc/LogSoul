...
function createSecurityRoutes(storage, authManager) {
    const router = (0, express_1.Router)();
    const security = new security_1.SecurityManager(storage);
    ...
    router.get('/events', async (req, res) => {
        try {
            const { page = 1, limit = 50, severity, type, start_date, end_date } = req.query;
    ...
    // IP management
    router.get('/ip-reputation/:ip', async (req, res) => {
        try {
            const { ip } = req.params;
    ...
    router.put('/waf/rules/:id', async (req, res) => {
        try {
            const { id } = req.params;
