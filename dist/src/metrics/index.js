...
class MetricsCollector {
    ...
    formatPrometheusMetrics(metrics) {
        ...
        const formatMetric = (name, help, type, values) => {
            ...
            if (typeof values === 'object' && values !== null) {
                for (const [label, value] of Object.entries(values)) {
                    if (typeof value === 'object') {
                        ...
                        for (const [subLabel, subValue] of Object.entries(value)) {
        ...
        // Format all metrics
        formatMetric('logsoul_requests_total', 'Total number of requests per domain', 'counter', metrics.logsoul_requests_total);
        formatMetric('logsoul_errors_total', 'Total number of errors per domain', 'counter', metrics.logsoul_errors_total);
        formatMetric('logsoul_response_time_avg', 'Average response time per domain in milliseconds', 'gauge', metrics.logsoul_response_time_avg);
        formatMetric('logsoul_response_time_95th', '95th percentile response time per domain in milliseconds', 'gauge', metrics.logsoul_response_time_95th);
        formatMetric('logsoul_domain_health_score', 'Health score per domain (0-100)', 'gauge', metrics.logsoul_domain_health_score);
        formatMetric('logsoul_security_threats_total', 'Number of security threats detected per domain and type', 'counter', metrics.logsoul_security_threats_total);
        formatMetric('logsoul_bandwidth_bytes', 'Total bandwidth usage per domain in bytes', 'counter', metrics.logsoul_bandwidth_bytes);
        formatMetric('logsoul_unique_ips', 'Number of unique IP addresses per domain', 'gauge', metrics.logsoul_unique_ips);
        formatMetric('logsoul_system_uptime', 'LogSoul system uptime in milliseconds', 'counter', metrics.logsoul_system_uptime);
        formatMetric('logsoul_system_memory_usage', 'LogSoul memory usage in bytes', 'gauge', metrics.logsoul_system_memory_usage);
        formatMetric('logsoul_domains_monitored', 'Number of domains being monitored', 'gauge', metrics.logsoul_domains_monitored);
        return output;
    }
