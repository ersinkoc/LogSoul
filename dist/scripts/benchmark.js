...
exports.LogSoulBenchmark = void 0;
...
class LogSoulBenchmark {
    constructor() {
        ...
        this.storage = new storage_1.Storage(':memory:'); // Use in-memory database for benchmarks
    ...
    async benchmarkLogParsing() {
        await this.benchmark('Nginx Log Parsing', 'Parse 10,000 Nginx combined format log lines', async () => {
    ...
    async benchmarkMultiFormatParsing() {
        await this.benchmark('Multi-Format Parsing', 'Parse different log formats (Nginx, Apache, JSON)', async () => {
            const samples = [
                {
    ...
    async benchmarkDatabaseOperations() {
        await this.benchmark('Database Operations', 'Add domains and fetch operations', async () => {
            let operations = 0;
            // Add domains
            for (let i = 0; i < 100; i++) {
    ...
    async benchmarkBulkInserts() {
        await this.benchmark('Bulk Log Inserts', 'Insert 1,000 log entries in batches', async () => {
    ...
    async benchmarkDomainAnalysis() {
        await this.benchmark('Domain Analysis', 'Analyze domain with 1,000 log entries', async () => {
            // Setup test data
    ...
    async benchmarkSecurityDetection() {
        await this.benchmark('Security Threat Detection', 'Detect threats in 1,000 malicious log entries', async () => {
    ...
    async benchmarkMemoryUsage() {
        await this.benchmark('Memory Efficiency', 'Process large dataset and measure memory growth', async () => {
...
exports.LogSoulBenchmark = LogSoulBenchmark;
